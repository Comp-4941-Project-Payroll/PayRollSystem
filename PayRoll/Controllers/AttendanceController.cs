...
namespace PayRoll.Controllers
{
    public class AttendanceController : Controller
    {
        ...
        {
            string curEmployee = Session["EmployeeId"].ToString();
            Attendance allAttendance = db.Attendances.Where(e => e.Employee.EmployeeId.Equals(curEmployee)).FirstOrDefault();
        ...
        public ActionResult PunchIn(Attendance model)
        {
            ...
            DateTime startTime = db.Schedules.Find(user.Shift).StartTime;
        ...
        public ActionResult PunchOut(Attendance model)
        {
            ...
            var todayLog = db.Attendances.Where(m => m.Employee.EmployeeId == user.EmployeeId && m.SignInTime == m.SignOutTime).FirstOrDefault();
            if (todayLog == null)
            {
                success = false;
                error = "You have not punched IN yet. Please try again.";
            }
